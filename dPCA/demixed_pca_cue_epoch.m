...
E = 30;    % maximal number of trial repetitions
time=1:T;
...
for n=1:N
    ...
    for s1=1:30
        spk_first_trials=all_trial_binned(s1,:);
      
            firingRates(n,1,:,s1)=spk_first_trials;
           
    end
    for s2=31:60
        spk_second_trials=all_trial_binned(s2,:);
            
            e=s2-30;
            firingRates(n,2,:,e)=spk_second_trials;
...
for n=1:N
    ...
    for t=1:length(Trials)
        trials_average=((1/bin)*1000)*sum(all_trial_binned(Trials{t},:))./30; 
        for e=1:length(Epochs) 
...
margNames = {'Condition', 'Main'};
margColours = [114 147 203; 241 51 139; 114 147 203; 241 51 139]/256;
...
[W,V,whichMarg] = dpca(firingRatesAverage, 25, ...
    'combinedParams', combinedParams, ...
...
accuracyShuffle = dpca_classificationShuffled(firingRates, trialNum, ...
    ...
    'simultaneous', ifSimultaneousRecording, ...
    'numRep', 10, ...        % increase to 100
    'numShuffles',10, ...  % increase to 100 (takes a lot of time)
